 ...
 *
 * @type {{name: string, hex: String, rgb: Number[]}[]}
 */
const cssColors = [
...
const cssColorNames = cssColors.map(color => color.name);
...
function parseColor(color, background) {
    ...
    if (
        hexColorRGB.test(colorLowercase) ||
        hexColorRRGGBB.test(colorLowercase) ||
        rgbColor.test(colorLowercase) ||
        cssColorNames.includes(colorLowercase)
    ) {
        if (background) {
            return { style: { backgroundColor: color, color: calculateContrastForeground(colorLowercase) } };
        } else {
...
/**
 * Calculates a fitting foreground color to have good contrast
 * to a given backgroundColor.
 *
 * @param {string} color given background color
 * @returns {string} foreground color, either `black` or `white`
 */
function calculateContrastForeground(color) {
    // Get R, G, B as (0..255)
    let r, g, b;

    if (hexColorRGB.test(color)) {
        ...
        g += g*16;
        b += b*16;
    } else if (hexColorRRGGBB.test(color)) {
        r = parseInt(color.substring(1, 3), 16);
        g = parseInt(color.substring(3, 5), 16);
        b = parseInt(color.substring(5, 7), 16);
    } else if (rgbColor.test(color)) {
        const match = color.match(rgbColor);

        r = parseInt(match[1], 10);
        g = parseInt(match[2], 10);
        b = parseInt(match[3], 10);
    } else if (cssColorNames.includes(color.toLowerCase())) {
        const cssColor = cssColors.find(c => c.name === color.toLowerCase());
    ...
    const l = calcRelativeLuminance(r, g, b);

    // Contrast ratios with black and white, take the better one.

    const lWhite = calcRelativeLuminance(1, 1, 1);
    const lBlack = calcRelativeLuminance(0, 0, 0);

    const contrastRatioWhite = calcContrastRatio(l, lWhite);
    const contrastRatioBlack = calcContrastRatio(l, lBlack);
 ...
 *
 * @param {Number} r Red in range 0.0..1.0
 * @param {Number} g Green in range 0.0..1.0
 * @param {Number} b Blue in range 0.0..1.0
 ...
 *
 * @param {Number} l1 Relative luminance of the first color
 * @param {Number} l2 Relative luminance of the second color
 * @returns {Number} contrast ratio in range 1.0..21.0
 */
function calcContrastRatio(l1, l2) {
    const lLighter = l1 > l2 ? l1 : l2;
    const lDarker = l1 > l2 ? l2 : l1;
